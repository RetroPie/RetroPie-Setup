...
function depends_bluetooth() { 
    getDepends bluez-utils bluez-compat bluez-hcidump blueman bluetooth
}

function list_available_devices_bluetooth() {	
    local line 
    local mac_address 
    local device_name 
    while read line; do 
        ...
        echo -e "$mac_address\n$device_name" 
    done < <(hcitool scan --flush | tail -n +2 | sed 's/\t/,/g') 
}

function list_registered_devices_bluetooth() {	
    local line 
    local mac_address 
    local device_name 
    while read line; do 
...
function display_active_and_registered_devices_bluetooth() {	
...
function remove_device_bluetooth() {	
    ...
    done < <(list_registered_devices_bluetooth)
...
function connect_device_bluetooth() {
    local mac_addresses=() 
    local mac_address 
    local device_names=() 
    local device_name
    ...
    while read mac_address; read device_name; do 
        ...
        device_names+=("$device_name") 
        options+=("$mac_address" "$device_name")
    done < <(list_available_devices_bluetooth)
    ...
    if [[ ${#mac_addresses[@]} -eq 0 ]] ; then
        ...
        exec 5>&1
        local step_successful
        step_successful=$(bluez-simple-agent hci0 "$choice" >&5)
        if [[ -z "$step_successful" ]] && [[ "$step_successful" != "Creating device filed: org.bluez.Error.AlreadyExists: Already Exists" ]] ; then
            step_successful=$(bluez-test-device trusted "$choice" yes >&5)
            if [[ -z "$step_successful" ]] ; then
                step_successful=$(bluez-test-input connect "$choice" >&5)
                if [[ -z "$step_successful" ]]; then
...
function configure_devices_bluetooth() { 
    while true; do
        ...
        if [[ -n "$choice" ]]; then 
            case $choice in 
                ...
                2) 
                    ...
                    ;; 
                3)
                    display_active_and_registered_devices_bluetooth
                    ;;
            esac 
        else 
            break 
