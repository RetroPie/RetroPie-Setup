...
function sources_mupen64plus() {
    rmDirExists "$rootdir/libretrocores/mupen64plus"
    # Base repo:
    # gitPullOrClone "$rootdir/libretrocores/mupen64plus" git://github.com/libretro/mupen64plus-libretro.git
    # Freezed fixed repo:
    gitPullOrClone "$rootdir/libretrocores/mupen64plus" git://github.com/gizmo98/mupen64plus-libretro.git
}

function build_mupen64plus() {
    rpSwap on 256 240
    
    pushd "$rootdir/libretrocores/mupen64plus"
    ...
    make clean
    make platform=rpi 
    if [[ -z `find $rootdir/libretrocores/mupen64plus/ -name "*libretro*.so"` ]]; then
...
function configure_mupen64plus() {
    ...
    # Copy config files
    cp $rootdir/libretrocores/mupen64plus/mupen64plus/mupen64plus-core/data/mupen64plus.cht $home/RetroPie/BIOS/mupen64plus.cht
    cp $rootdir/libretrocores/mupen64plus/mupen64plus/mupen64plus-core/data/mupencheat.txt $home/RetroPie/BIOS/mupencheat.txt
    cp $rootdir/libretrocores/mupen64plus/mupen64plus/mupen64plus-core/data/mupen64plus.ini $home/RetroPie/BIOS/mupen64plus.ini 
    cp $rootdir/libretrocores/mupen64plus/mupen64plus/mupen64plus-core/data/font.ttf $home/RetroPie/BIOS/font.ttf
    ...
    setESSystem "Nintendo 64" "n64" "~/RetroPie/roms/n64" ".z64 .Z64 .n64 .N64 .v64 .V64" "$rootdir/supplementary/runcommand/runcommand.sh 1 \"$emudir/$1/bin/retroarch -L `find $rootdir/libretrocores/mupen64plus/ -name \"*libretro*.so\" | head -1` --config $rootdir/configs/all/retroarch.cfg --appendconfig $rootdir/configs/n64/retroarch.cfg $__tmpnetplaymode$__tmpnetplayhostip_cfile $__tmpnetplayport$__tmpnetplayframes %ROM%\"" "n64" "n64"
}
