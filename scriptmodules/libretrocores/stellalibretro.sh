...
function sources_stellalibretro() {
    gitPullOrClone "$rootdir/libretrocores/stella-libretro" git://github.com/libretro/stella-libretro.git
}

function build_stellalibretro() {
    pushd "$rootdir/libretrocores/stella-libretro"
    make clean
    make
    if [[ -z `find $rootdir/libretrocores/stella-libretro/ -name "*libretro*.so"` ]]; then
...
function configure_stellalibretro() {
    ...
    setESSystem "Atari 2600" "atari2600-libretro" "~/RetroPie/roms/atari2600-libretro" ".a26 .A26 .bin .BIN .rom .ROM .zip .ZIP .gz .GZ" "$rootdir/supplementary/runcommand/runcommand.sh 1 \"$emudir/$1/bin/retroarch -L `find $rootdir/libretrocores/stella-libretro/ -name \"*libretro*.so\" | head -1` --config $rootdir/configs/all/retroarch.cfg --appendconfig $rootdir/configs/atari2600/retroarch.cfg $__tmpnetplaymode$__tmpnetplayhostip_cfile$__tmpnetplayport$__tmpnetplayframes %ROM%\"" "atari2600" "atari2600"
}