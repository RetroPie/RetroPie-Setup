...
function configure_lr-gambatte() {
    ...
    setESSystem "Game Boy" "gb" "~/RetroPie/roms/gb" ".gb .GB .zip .ZIP" "$rootdir/supplementary/runcommand/runcommand.sh 0 \"$emudir/retroarch/bin/retroarch -L $md_inst/gambatte_libretro.so --config $configdir/gb/retroarch.cfg %ROM%\" \"$md_id\"" "gb" "gb"
    setESSystem "Game Boy Color" "gbc" "~/RetroPie/roms/gbc" ".gbc .GBC .zip .ZIP" "$rootdir/supplementary/runcommand/runcommand.sh 0 \"$emudir/retroarch/bin/retroarch -L $md_inst/gambatte_libretro.so --config $configdir/gbc/retroarch.cfg %ROM%\" \"$md_id\"" "gbc" "gbc"
}
